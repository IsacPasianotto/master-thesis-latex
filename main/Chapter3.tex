...
% This chapter is dedicated to explaining in detail the criteria and
...
% the execution of each of these benchmark will be repeated 30 times, in order
...
% Another option that has to be verified is if the change in the packet size
% triggers a change   TCP -> UDP in the communication.
...
% These minor differences make it so that when reasoning in terms deploying of a
% general-purpose infrastructure with no particular hints about the kind of
% workload that will host other features, such as ease of deployment, policy
% handling, and stability, shoud also be carefully  considered
